...
import { randomIntFromInterval, reverseLinkedList, useInterval } from '../lib/utils';
import { db } from '../firebase/firebase-config';
import { updateDoc, doc, where, collection, getDocs, query } from 'firebase/firestore';
...
export default function Main( { records, onUpdateRecord } ) {
    ...
    const compareScore = () => {
        ...
        getDocs(usuariosQuery).then((usuariosSnapshot) => {
          usuariosSnapshot.forEach((doc) => {
            const userData = doc.data();
            const recordInt = userData.record;
    ...
    return (
        <div className="general">
            ...
            <button className='pause' onClick={() => setIsPaused(!isPaused)}>
                {isPaused ? 'Play' : 'Pause'}
            </button>
        </div>
    )
...
const getCellClassName = (
        ...
        if (foodShouldReverseDirection) {
            className = 'cell food-cell2';
        } else {
            className = 'cell food-cell1';
        }
        }
